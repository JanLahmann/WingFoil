...
```
0.9.11: every buzz also paints. A short flash with the verdict for every jibe and tack (FLEW / TOUCH / FELL), the star for a clean jibe, a dry streak of 5, 10, 20 …, a new longest flight; a ring for a pumped takeoff; then a line at the top of every page keeps the last verdict for 20 s. Switch: "Show alerts on screen". Also: the fenix 5 Plus, 5S Plus and 5X Plus join the list; "Reset pages to defaults" in the settings puts the seven pages back; the Turns page header names its counts (flew · touch · fell), the main page says them in words where there is room, the start page says START records · BACK saves, and the summary says NOT SAVED if the file was not written.
