...
| 0.9.14-dev5 | 2026-09-19 | — | **The stream carries the clock.** Schema 2: a 20-byte header with the watch's UTC offset in minutes, because the first direct session (16:52) landed at 15:52 — the phone had only the longitude to guess by, which is the solar offset and an hour out under summer time. Pair with iPhone dev build 87, which reads both schemas. |
