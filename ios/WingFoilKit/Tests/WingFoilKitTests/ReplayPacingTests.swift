...
@Suite struct ReplayPacingTests {
    ...
    /// At twenty-five seconds the firsts and the best streak come back; at sixty every line
    /// survives, because twelve lines is what a minute has room for and this afternoon has
    /// exactly twelve to say.
    @Test func eachLengthKeepsTheMomentsItHasRoomFor() throws {
        ...
        #expect(ReplayPacing.plan(span: span, targetWallS: 25, milestones: milestones)
            .milestones.map(\.id) == [
                ...
                // firsts — which since engine 0.10.0 includes the first **clean** jibe. Under
                // engine 0.14.0 that clean jibe is the 3rd dry one, so it arrives inside
                // `streak-3` rather than as a line of its own, and `streak-3` takes the slot.
                "start", "longest-flight", "jibe-1", "streak-3", "top-speed", "streak-8",
                "splash-1", "end",
            ])

        // A minute has room for twelve lines, and under 0.14.0 this afternoon has exactly
        // twelve to say — the clean-jibe beat merged into a streak record — so a sixty-second
        // clip is the first length that leaves nothing out.
        #expect(ReplayPacing.plan(span: span, targetWallS: 60, milestones: milestones)
            .milestones.map(\.id) == milestones.map(\.id))
    }
