...
/// a track stretched across the full width leaves the words a 200 px strip, and the title
/// reads as a caption under a banner. Beside the words the same track is square-ish — same
/// tokens, same type sizes, only the axis changes (`ShareCardStats.Shape.isWide`).
///
/// **Layout B v2 on both cards** (Jan, 26 Sep 2026): the session card and the period card
/// draw the same story — hero, outcome bars, streak line, ribbon — packed from the footer
/// up, and the track takes every point the words do not need. The tile grid that used to
/// run past the footer on a landscape period card is gone.
struct ShareCardView: View {
    ...
    private var mapScrim: some View {
        ...
        LinearGradient(
            ...
            startPoint: .top, endPoint: .bottom)
        if shape.isWide {
            let edge = (size.width * (1 - Self.storyColumn) - 16) / size.width
            LinearGradient(
    ...
    private var content: some View {
        Group {
            // Every card this view draws has a story (layout B v2); the clip's closing card,
            // the one `ShareCardStats` without one, has a view of its own.
    ...
    /// the date, and it costs the track about fourteen points of height. Absent, the header is the two lines it has always been and nothing moves.
    ///
    ...
    // outcome bars · the streak line · one ribbon · the footer — the twin of `storyBoxes` /
    // `drawStory` in
    ...
    private func squareTrackBeside(_ story: ShareCardStats.Story) -> Bool {
        ...
        func scale(_ w: CGFloat, _ h: CGFloat) -> Double {
            if !extents.isEmpty {
                return TrackStack.placement(
                    of: extents, in: TrackStack.Box(x: 0, y: 0, w: Double(w), h: Double(h)),
                    inset: ShareCardStats.trackInset)?.scale ?? -1
            }
            guard let box else { return -1 }
            let dx = max(box.maxX - box.minX, 1e-6), dy = max(box.maxY - box.minY, 1e-6)
            return min(Double(w - inset) / dx, Double(h - inset) / dy)
        }
