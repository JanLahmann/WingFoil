...
│                               find — are the first and last sections of /start/ now, and
│                               a reader met "Get the beta" and "Get started" as two
│                               destinations for one job (Jan, from his phone). The
...
│                               is the port"). The four tabs the phone has, in its order and
│                               its words — Sessions · Records · Trends · Gear & spots — with
│                               the app menu on every one of them (What CleanJibe does ·
│                               Getting started · Settings · Help · Support & ideas) and the
│                               session page behind a row, cut into Ride · Turns · Takeoffs ·
│                               Log. The analyzer's own strengths — no install, drop a file,
│                               one-click example — are the first two rows of the Sessions
│                               tab's empty state rather than a view called Analyze.
│                               docs/copy/app-shell.json carries the three lists and
│                               tools/verify_app_shell.py holds them to the kit's own Swift.
│                               "Lab" and the module name are gone from every rider-visible
│                               string.
...
├── css/app.css                 the app's shell: the menu button, the tab bar (a rail above
│                               860 px), the pages, the ways-in card and the session rows.
│                               Layered on style.css the way home.css is, and it defines no
│                               colour of its own, so the light theme carries it
├── js/app.js                   file intake, progress, save-to-library, SW updates
├── js/appshell.js              the SHELL: the four tabs, the app menu, routing, the welcome,
│                               Settings, Help and Gear & spots. Decides nothing about a
│                               number; the one piece of arithmetic in it is the 500 m spot
│                               clustering, which is the rule lab_bundle/library.py uses
├── js/appsettings.js           what this browser remembers: the speed unit, the welcome's
│                               flag, a gear name per session, and HOW MUCH EVERY
│                               EXPLANATION SAYS (concise or extensive, concise by default)
├── js/explain.js               CONCISE OR EXTENSIVE, site-wide (20 Sep 2026). Anything
│                               carrying data-explain="<help topic id>" keeps its own one
│                               line and gains a `?` into #/help/<id>; extensive draws the
│                               topic's BODY under it, read from HELP at render time so
│                               there is never a second copy (pattern F). A surface whose
│                               detail is generated — the turn page's footnote, which
│                               quotes this session's own thresholds — hands its own
│                               .explain-more in and this only shows and hides it. A
│                               MutationObserver wires whatever js/session.js and
│                               js/turnpage.js draw later
├── js/appcopy.js               GENERATED by tools/make_app_copy.py — the shell's lists, the
│                               ways in, the welcome, What's new, Help, the feedback door.
│                               Do not edit
...
│                               two pages that print them: / and /start/.
...
│                               or more appears on two of the three prose pages, and no
...
│                               outputs
├── tools/make_app_copy.py      writes js/appcopy.js from docs/copy: the shell's three
│                               lists, the ways in, the welcome, What's new, Help, the
│                               feedback door, and SETTINGS_SECTIONS — every Settings
│                               section the browser draws, in the order SettingsView draws
│                               it, out of docs/copy/settings.json (the kit's SettingsCopy,
│                               exported by SettingsCopyExportTests). `--check` exits 1 if
│                               it is stale, and runs with verify_links.py. Stdlib only
...
- **Two destinations and two doors** since 19 September 2026 (Jan: *"one product, three
  ...
  beside them is *Open the app* (`/app/`); *Get the beta* came out on 20 September 2026,
  when `/invite/` folded into `/start/`. The row was four
