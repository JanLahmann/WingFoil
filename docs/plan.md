...
- **map** = the session's own breadcrumb, drawn by `RecordingView` like every other page (`source/ui/TrackDraw.mc`) from the decimated 128-point lat/lon buffer in `MetricsEngine`, which is filled unconditionally. It was `WatchUi.MapTrackView` — the firmware's native map view — until 0.9.2, and it killed the app on Jan's fenix 8 through both documented ways of putting a native view on screen (`switchToView` → Type Error, `pushView` → a silent firmware kill with nothing in CIQ_LOG). Self-drawn, it needs no firmware map support, carries the PAUSED banner, is measurable by the layout suite, and exists on every product. See `docs/watch-ui-review.md` §12.1.
...
- **The breadcrumb is tinted by foil state**: the phase teal while flying, the dim ink otherwise. The track is split into one run per stretch of equal state (`TrackTint`, now in `source/ui/TrackDraw.mc`) so the renderer sets a colour once per run rather than once per segment. A minimum run length grows until the run count fits `MAX_RUNS` (32), which absorbs single-point flicker — at ~1 decimated point per minute that is the honest resolution anyway. The live map page and the post-save Track page call the same renderer.
- **Launcher icons ship at each product's native size** (`compiler.json` `launcherIcon`). Four sizes now: **65 px** — fenix 8 47 mm / 8 Pro, fr965, fr970, fr570 47 mm, D2 Mach 2 — is what `resources/` holds and takes no override; **60 px** — fenix 8 43 mm, the whole epix 2 / epix 2 Pro family, fr265, both MARQ 2, D2 Mach 1, Descent Mk3 43 mm — is `resources-icon60`; **40 px** — both fenix 8 Solars, all eight fenix 7 variants, fr255, fr955, Enduro 3 — is `resources-icon40`; and **54 px**, which exactly one product asks for (`fr57042mm`, whose 47 mm sibling wants 65), is `resources-icon54`. Each is a Lanczos resample of `brand/icon-tile-1024.png`, the master the other three came from. Per-product `resourcePath` lines in every jungle (app and field, release, beta and invite) map product to directory; exports build with zero rescale warnings.
- **Thirty products, one list across app, field and barrel** (0.9.4, ADR-014): the fenix 8 five, the fenix 7 eight, and the seventeen Tier A watches — epix 2 / epix 2 Pro 42 / 47 / 51 mm, fr255, fr265, fr570 42 / 47 mm, fr955, fr965, fr970, MARQ 2, MARQ 2 Aviator, Enduro 3, D2 Mach 1, D2 Mach 2, Descent Mk3 43 mm. Every channel of both apps now advertises the same watches, which the release data field did not before. Note that several ids cover more watches than they name: `fenix7x` **is** Enduro 2, tactix 7 and quatix 7X Solar, `fenix847mm` is tactix 8 and quatix 8, `epix2pro51mm` is tactix 7 AMOLED and D2 Mach 1 Pro, and `epix2` is quatix 7 Sapphire — so the store listing's "Enduro 2 and tactix 7/8" was always true, just not visible in the manifest.
...
- **Persistence: GRDB + immutable archive** (not SwiftData — workloads are SQL aggregations, background imports, migration control; no CloudKit need). `Sessions/<uuid>/original.fit` immutable + `analysis.json` (versioned `SessionAnalysis`; engine bump ⇒ lazy re-analysis) + `wingfoil.sqlite`: `session` (denormalized summary cols), `flight`, `turn`, `takeoff_attempt`, `record_effort` (PB history), `gear`, `spot`, `import_log`. Dedupe key = startDate ±60 s + duration ±60 s (same session may arrive via icu, GDPR bulk, AirDrop). **Library backup** (ADR-015, Settings → Library backup): one zip — `manifest.json` + a `VACUUM INTO` snapshot of the database + the whole `Sessions/` tree — written to `tmp/` and handed to the share sheet; restore re-ingests every session through the *same* ingest/dedupe path and merges the metadata additively (never a file-level copy over the live database), so it is idempotent and cannot overwrite a newer local edit.
...
- **Features:** Library (rows carry a cached track thumbnail + speed sparkline + outcome tally) · Session Detail (MapKit track colored by phase/speed, record-window glow, Swift Charts speed timeline w/ flight bands + turn markers + pump subchart, divergence banner, **replay scrubber** whose playhead is shared by chart and map so tapping either moves both, **share card** composer) · Records (all-time/per-spot/per-gear, kn; medal by record freshness, confetti + haptic on a new all-time PB) · Trends (foil %, longest flight, turn success, pumps-to-takeoff, port/starboard) · Gear · Spots (auto-cluster + wind roses) · Import (icu polling w/ Keychain key; Files/share-sheet FIT/GPX/ZIP; GDPR nested-ZIP backfill with sport sniff + dedupe + progress) · **New-session notifications** (opt-in, off by default: a `BGAppRefreshTask` polls intervals.icu, anything unseen and not already in the library becomes a local notification, the FIT is prefetched and analysed in the remaining background time, and the tap opens the session — decision logic in `NewActivityWatch`, glue in `ActivityNotifier`; no server, so no push) · Wind (Open-Meteo enrich + estimator reconcile) · **Help** (plain-language explanation of every metric, deep-linked from a `?` on the card that shows it) · **Widgets** (last session + weekly foil time; see ADR-011 on the app group) · Settings (thresholds mirroring `docs/algorithms.md`, re-analyze button, attribution).
...
- **Phase 6 — Store-readiness (optional):** Beta App → store listing, intervals.icu OAuth2, MIP-variant QA, DE localization, pricing decision — worked out in the private artifact *CleanJibe Pro* (5 Sep 2026); like the competitive research and the replay plan, it stays out of the public repo.
