...
- **Launcher icons ship at each product's native size** (`compiler.json` `launcherIcon`: 65 px for fenix 8 47 mm / 8 Pro, 60 px for 43 mm, 40 px for both Solars). `resources/` holds the 65 px source and `resources-icon60` / `resources-icon40` override it through per-product `resourcePath` lines in every jungle (app and field, release and beta). Exports build with zero rescale warnings.
