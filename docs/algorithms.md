...
`ENGINE_VERSION`: **0.19.0** (bump on any change that alters outputs; triggers phone re-analysis)
...
the way tuning's fingerprint does — `0.19.0+disc.windsurfFin`, composing inside the tuning
stamp (`0.19.0+disc.windsurfFin+tuned.2.a1b2c3d4`) — so switching one session's discipline
