...
| Sessions | **Flight-end page** | the same page for the losses no turn owns | — | swipe for the next · Done | release | same, opened from a **Flight ends** row under the Flights chip, as the phone opens it from a flight row. The same set the phone draws: no end a turn owns, no end the recording truncated | — |
...
the four sub-tabs **Ride · Turns · Flights · Details**, as a switcher on a narrow screen
...
23. **A Settings section a browser cannot do says so instead of being absent.** Since
    ...
    order; Notifications, Analysis and Session list carry the phone's header and
...
| **Help** | the topics, from the kit's help export, in the same seven sections, as a chip index over seven shut folds with a filter | the filter's *0 pages match.* | a topic |
