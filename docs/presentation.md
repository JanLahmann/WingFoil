...
1. **Getting started** — the beta test guide as a help topic (`HelpTopicID.betaGettingStarted`,
   ...
   verdicts against what you remember, share a card, Menu → Support & ideas — and the 20-minute dry
   run is kept as "the minimal check, if you cannot wait" after it. The help topic still leads
   with the dry run; bringing the two back into step is an iOS change. Its title is plain
   **Getting started** since 14 September 2026 — it read *"Getting started (beta test)"* and
   opened *"CleanJibe is a beta"*, which is the first row of the menu in the App Store build
   too. The TestFlight route stays in it, conditionally: *"TestFlight's own feedback if you
   are on the beta"*.
