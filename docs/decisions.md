...
**Removed 0.9.19** — the code, not the status: `LockView.mc` and `LockGate.mc` (`garmin/source/lock/`), the six `Lock*` strings in `docs/copy/watch.json`, `WingfoilApp`'s two dead branches that reached them (`getInitialView`'s lock check, `onSettingsChanged`'s re-validation and the `unlockedNow()` it called), and the five tests that named them, all unreachable since the lock retired above. `lab/tools/make_unlock.py` and its test vectors are untouched, so the scheme can still be reimplemented from this ADR if the gate is ever re-armed. `source-nopepper/UnlockPepperDefault.mc` was left on every jungle's source path — a harmless unused module now rather than a second file to re-wire a consumer into. No jungle at `garmin/` top level has named `monkey-invite.jungle` for some time (only `manifest.xml`, `manifest-beta.xml` and `manifest-dev.xml` exist there today, all three on `source-nopepper/`), and `garmin/tools/package.sh` has never referenced `garmin/gen/` or a pepper — so the untracked `garmin/gen/UnlockPepper.mc` some clones still carry was already orphaned before this round, not made so by it.
