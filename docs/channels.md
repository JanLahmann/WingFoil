...
| feature | channel | note |
|---|---|---|
| Engine 0.20.0: foil time, flights, touchdowns, falls, wind axis | release | the number is the engine the three channels share; `docs/algorithms.md` is the contract and `tools/check_release.py` holds this row to it |
...
| feature | channel | |
|---|---|---|
| **Page set: standard / large text** — one Garmin Connect enum, five big screens or the seven standard ones (docs/presentation.md, "The watch's two page sets") | **release** | it is one enum with two values, it needs no explanation, and it is the answer to the one piece of feedback that started the round ("I need my glasses"). A rider who never opens it gets exactly the app he had |
| **The per-page editor** — seven screens, each a layout plus five metric slots, plus *Reset pages to defaults* (`pg1Layout` … `pg7s5`, `resetPages`) | **dev** | 43 rows in Garmin Connect for a rider who mostly wants one thing bigger. It is a tuning instrument, and it moved out of `resources/settings/` into `resources-dev/base/settings/` in this round along with its strings; `PageModel._store` is the `(:notdev)` twin that answers with the default table, so release and beta show the seven pages they always showed. **Candidate for beta the day a rider asks for it**: one jungle line moves the resources, and the four rules are already met on the code (it shipped to every rider from 0.8.2 to 0.9.15 with no report against it) — what it lacks is anyone asking |
| **The direct transfer's progress line and its buzz** — `phone 4/13` on the SAVED and start screens, one tick when the stream is whole | **dev** | it rides with the direct transfer itself (below), and moves when that does |
